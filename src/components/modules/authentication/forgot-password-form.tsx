...
export function ForgotPasswordForm({ className, ...props }: React.ComponentProps<typeof Card> & { className?: string }) {
  const router = useRouter();

  const form = useForm({
    defaultValues: { email: "" },
    validators: { onSubmit: forgotPasswordSchema, onChange: forgotPasswordSchema },
    onSubmit: async ({ value }) => {
      const toastId = toast.loading("Sending reset code...");
      try {
        const res = await fetch("/api/v1/auth/forgot-password", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: value.email }) });
        const data = await res.json();
        if (!res.ok) { toast.error(data?.message || "Something went wrong", { id: toastId }); return; }
        toast.success("A password reset OTP has been sent to your email.", { id: toastId });
  ...
  return (
    <Card className={cn("card-3d w-full max-w-md border-border/40 bg-card/90 backdrop-blur-2xl rounded-2xl shadow-[0_8px_40px_-12px_rgba(124,58,237,0.15),0_4px_16px_-4px_rgba(0,0,0,0.08)]", className)} {...props}>
      <CardHeader className="space-y-3 text-center pb-6 pt-10">
        <div className="mx-auto mb-2 flex size-16 items-center justify-center rounded-2xl bg-gradient-to-br from-[#f59e0b] via-[#ec4899] to-[#7c3aed] shadow-xl shadow-[#ec4899]/30 -rotate-3 hover:rotate-0 transition-transform duration-300">
        ...
        <CardDescription className="text-[15px] leading-relaxed">Enter your email address and we&apos;ll send you an OTP to reset your password.</CardDescription>
      </CardHeader>

      <CardContent className="space-y-5 px-8">
        <form id="forgot-password-form" className="space-y-4" onSubmit={(e) => { e.preventDefault(); e.stopPropagation(); form.handleSubmit(); }}>
          ...
          <Button form="forgot-password-form" type="submit" size="lg"
            className="btn-3d w-full mt-2 h-12 font-bold text-[15px] rounded-xl bg-gradient-to-r from-[#f59e0b] via-[#ec4899] to-[#7c3aed] text-white hover:opacity-95 transition-all"
            disabled={form.state.isSubmitting || !form.state.canSubmit}>
            {form.state.isSubmitting ? (<><Loader2 className="mr-2 h-5 w-5 animate-spin" />Sending...</>) : "Send Reset OTP"}
          </Button>
        </form>
      </CardContent>
